...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	/**
	 * 本地线程缓存, 记录正在被加载的资源, 主要为了避免一个 EncodedResource 在加载时，还没加载完成，又加载自身，从而导致死循环
	 */
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		//如果没办法添加到本地线程缓存中, 则表示该资源正在被加载
		if (!currentResources.add(encodedResource)) {
			throw new BeanDefinitionStoreException(
					"Detected cyclic loading of " + encodedResource + " - check your import definitions!");
		}
		try {
			//获取 Resource 的 InputStream
			InputStream inputStream = encodedResource.getResource().getInputStream();
			try {
				//创建 InputSource
				InputSource inputSource = new InputSource(inputStream);
				//如果 EncodedResource 中有指定 encoding
	...
	protected int getValidationModeForResource(Resource resource) {
		...
		//然后, 自动获取验证模式, 也就是XML上配置的验证模式
		int detectedMode = detectValidationMode(resource);
		if (detectedMode != VALIDATION_AUTO) {
			return detectedMode;
		}
		//最后, 使用 VALIDATION_XSD 验证模式作为默认
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		...
		//获取注册器, 然后统计已经存在的 BeanDefinition 的数量
		int countBefore = getRegistry().getBeanDefinitionCount();
		//创建 XmlReaderContext 对象
		//注册 BeanDefinition
		documentReader.registerBeanDefinitions(doc, createReaderContext(resource));
		//计算出已经添加的 BeanDefinition 的个数
	...
	protected BeanDefinitionDocumentReader createBeanDefinitionDocumentReader() {
		//根据 documentReaderClass 创建 BeanDefinitionDocumentReader 对象
